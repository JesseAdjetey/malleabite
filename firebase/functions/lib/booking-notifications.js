...
exports.onBookingCreated = (0, https_1.onCall)({ secrets: [smtpUser, smtpPass, smtpHost, appBaseUrl] }, async (request) => {
    ...
    if (!emailRegex.test(data.guestEmail)) {
        throw new https_1.HttpsError('invalid-argument', 'Invalid guest email address');
    }
    ...
    const hostName = hostData?.displayName || 'Your host';
    ...
    emailPromises.push(transport.sendMail({
