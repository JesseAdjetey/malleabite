...
const vapiServerSecret = defineSecret('VAPI_SERVER_SECRET');

// Convert OpenAI message roles to Gemini roles
function toGeminiHistory(messages: any[]): { role: string; parts: { text: string }[] }[] {
  const history: { role: string; parts: { text: string }[] }[] = [];

  for (const msg of messages) {
    if (msg.role === 'system') continue; // system handled separately as instruction
...
function sseChunk(id: string, delta: string, finish: string | null = null): string {
  const payload = {
    id,
...
export const vapiLlm = onRequest(
  {
    cors: true,
    region: 'us-central1',
    memory: '512MiB',
  ...
  async (req, res) => {
    ...
    const historyMessages = messages.filter((m: any) => m.role !== 'system').slice(0, -1);
