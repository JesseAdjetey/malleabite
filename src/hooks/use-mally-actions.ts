  ...
  const executeAction = useCallback(async (action: { type: string; data: any }): Promise<boolean | string> => {
    try {
      ...
      switch (type) {
        ...
        case 'delete_event': {
          const evToDel = resolveEvent(data);
          if (!evToDel) { toast.error(`Couldn't find event${data.title ? ` "${data.title}"` : ''}`); return false; }
          const result = await removeEvent(evToDel.id);
          if (result?.success) { toast.success(`Deleted "${evToDel.title}"`); return true; }
          return false;
        }
